...
import { addCommentBlog, fetchPostCategories, fetchSingleBlog } from "@/src/redux/action/blogAction";
...
const Blog = (props) => {
    ...
    const postComments = useSelector(state => state.blog.postComments);
    ...
    const handleAddComment = () => {
        ...
        params.post_id = blogDetails?.post_id
    ...
    return (
        <div className="blog-page pb-5">
            <h2 className="pagemain-title py-5">Blog</h2>
            <div className="row justify-content-center m-0">
                <div className="col-lg-8 col-sm-10 col-12 px-sm-2 px-3">
                    <div className="row">
                        <div className="single-blog-img">
                            <img src={`https://api.viannejewels.com/${blogDetails?.post_feature_image}`} class="d-block w-100" alt="..." style={{ height: 455, objectFit: 'cover' }} loading="lazy" />
                        </div>
                                                ...
                                                {postComments?.map((post,i) => {
                                                    return (
                                                        <div className="comment-data" key={i}>
                                                            <p className="commenter-comment">{post?.content}</p>
                                                            <div className="d-flex align-items-center justify-content-end">
                                                                <a href="#" className="commenter-name">{post?.author}</a>
                                                                <a href="#" className="commenter-date ms-3"> {moment(post?.date).format('MMM Do YYYY')}</a>
                                                            </div>
                                                        </div>
                                                    )
                                                })}
                            ...
                            <div className="cateogory-title-data">
                                        ...
                                        return (
                                            <div class="accordion-item border-0" key={index}>
                                                ...
                                                <div id={`flush-collapseOne${index}`} className="accordion-collapse text-start collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
                                                    <div className="py-2 px-0 accordion-body pointer" onClick={() => handlePostByCategory(d?.id)}> {d?.slug} ({d?.count})</div>
                                                </div>
                                            </div>
                                        )
                                    })}
                                ...
                                <ul className="latest-post-list">
                                    {blogList?.slice(0, 5)?.map((d,i) => {
                                        return (
                                            <li className="latest-post-box" onClick={() => handleRecentPost(d)} key={i}>
                                                <img src={d?.post_feature_image} alt="img" loading="lazy" />
